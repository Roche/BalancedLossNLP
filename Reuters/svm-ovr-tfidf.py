...
corpus_train = [docu['text'] for docu in data_train]
pipe = Pipeline([('count', CountVectorizer()),
                 ('tfidf', TfidfTransformer())]).fit(corpus_train)

X_all = pipe.transform([docu['text'] for docu in data_train]).toarray()
y_all = np.array([[1 if x in docu['labels'] else 0 for x in labels_ref] for docu in data_train])
pickle.dump((X_all, y_all), open(os.path.join(source_dir, 'data', 'data_train.'+suffix+'.tfidf'),'wb'))

X_all = pipe.transform([docu['text'] for docu in data_val]).toarray()
y_all = np.array([[1 if x in docu['labels'] else 0 for x in labels_ref] for docu in data_val])
pickle.dump((X_all, y_all), open(os.path.join(source_dir, 'data', 'data_val.'+suffix+'.tfidf'),'wb'))

X_all = pipe.transform([docu['text'] for docu in data_test]).toarray()
y_all = np.array([[1 if x in docu['labels'] else 0 for x in labels_ref] for docu in data_test])
pickle.dump((X_all, y_all), open(os.path.join(source_dir, 'data', 'data_test.'+suffix+'.tfidf'),'wb'))
...
miF_test = f1_score(y_test, y_test_score>best_micro_f1_th, average='micro', zero_division=0)
maF_test = f1_score(y_test, y_test_score>best_micro_f1_th, average='macro', zero_division=0)
res_model.extend(['one-weight-shift-hyper-pane', best_micro_f1_th])
res_model.extend([miF_val, maF_val, miF_test, maF_test])
...
miF_test = f1_score(y_test, y_test_score>best_micro_f1_th, average='micro', zero_division=0)
maF_test = f1_score(y_test, y_test_score>best_micro_f1_th, average='macro', zero_division=0)
res_model.extend(['balanced-weight-shift-hyper-pane', best_micro_f1_th])
res_model.extend([miF_val, maF_val, miF_test, maF_test])
